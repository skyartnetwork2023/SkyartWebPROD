...
export function SiteFooter() {
  ...
  const onSubscribe = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      form.reset();
      toast.success(res.existing ? messages.alreadySubscribed : messages.subscribedSuccess);
    } catch (err) {
      toast.error((err as Error).message || messages.subscribeFailed);
    } finally {
      setLoading(false);
    }
  };
    ...
    <footer className="border-t bg-surface">
      <div className="container-page py-16">
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-5">
          <div className="lg:col-span-2">
            ...
            <form onSubmit={onSubscribe} className="mt-6 flex max-w-sm gap-2">
              <Input name="email" type="email" placeholder={messages.yourEmail} aria-label={messages.newsletterAria} required />
              <Button type="submit" disabled={loading}>{loading ? "…" : messages.subscribe}</Button>
            </form>
            <SocialLinks />
          ...
          <div>
            <h4 className="font-display text-sm font-semibold uppercase tracking-wider">{messages.company}</h4>
            <ul className="mt-4 space-y-2 text-sm">
              <li><Link to="/about" className="text-muted-foreground hover:text-primary">{messages.about}</Link></li>
              <li><Link to="/portfolio" className="text-muted-foreground hover:text-primary">{messages.portfolio}</Link></li>
              <li><Link to="/careers" className="text-muted-foreground hover:text-primary">{messages.careers}</Link></li>
              <li><Link to="/blog" className="text-muted-foreground hover:text-primary">{messages.blogNews}</Link></li>
              <li><Link to="/contact" className="text-muted-foreground hover:text-primary">{messages.contact}</Link></li>
            </ul>
          </div>

          <div>
            <h4 className="font-display text-sm font-semibold uppercase tracking-wider">{messages.services}</h4>
            <ul className="mt-4 space-y-2 text-sm">
              <li><Link to="/services" className="text-muted-foreground hover:text-primary">{messages.allServices}</Link></li>
              <li><Link to="/packages" className="text-muted-foreground hover:text-primary">{messages.packages}</Link></li>
              <li><Link to="/solutions" className="text-muted-foreground hover:text-primary">{messages.solutions}</Link></li>
              <li><Link to="/coverage" className="text-muted-foreground hover:text-primary">{messages.coverage}</Link></li>
              <li><Link to="/faq" className="text-muted-foreground hover:text-primary">{messages.faq}</Link></li>
            </ul>
          </div>

          <div>
            <h4 className="font-display text-sm font-semibold uppercase tracking-wider">{messages.reachUs}</h4>
        ...
        <div className="mt-12 flex flex-col-reverse items-start justify-between gap-4 border-t pt-6 text-xs text-muted-foreground md:flex-row md:items-center">
          <p>© {new Date().getFullYear()} {site.name}. {messages.footerRights}</p>
          <div className="flex gap-4">
            <a href="#" className="hover:text-primary">{messages.privacyPolicy}</a>
            <a href="#" className="hover:text-primary">{messages.termsConditions}</a>
            <a href="#" className="hover:text-primary">{messages.cookies}</a>
          </div>
        </div>
      </div>
    </footer>
